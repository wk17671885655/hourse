...
<SCRIPT type="text/javascript" src="/admin/js/jquery-1.8.3.js"></SCRIPT>
  <script type="text/javascript">
    $(function () {
        $("#btu1").click(function () {
            $.post("checkName",{"username":$("#name").val()},function (data) {
                // alert(data.result);
                if(data.result==1){
                    $("#notice").html("该用户名已存在").css("color","red");
                }else {
                    $("#notice").html("可以使用").css("color","green")
                }
            },"json")
          // $.post("checkName",{"username":$("#name").val()}),function (data) {
          //     alert(data.result);
...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><INPUT class=text type=text name=name id="name"><span id="notice"></span></TD>
     <TD><input type="button" value="检查用户名是否存在" id="btu1"></TD>
  </TR>
  <TR>
